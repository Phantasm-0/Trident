...
def stock(message):
    ...
    if ((re.search("Встреча", text) is not None) or (re.search("Deposited", text) is not None) or (
            re.search("Ты заметил враждебных существ", text) is not None) or (
            re.search("Получено:", text) is not None)):
        return
...
def give_any(message):
    ...
    if (answer == g_withdraw):
        result_list = re.findall(r"\w*\d{1,3}\s\d{1,3}",  text)
...
def give_all_(regex, list_for_search, g_what, answer_name):
    ...
    while (len(list_for_search) > 9):
        ...
        answer += give_additional_all(regex, additional_any, g_what, answer_name)
